...
\paragraph{Situation can occur only on the Asset/Bond market.} An agent can hold a mix of cash, free assets and positive bonds or collateralized assets. To show that the situation can occur only on the Asset/Bond market one must investigate the combinations of allocations and possible trading options.
...
\paragraph{Formally}
It is of very importance to note that an agent can never have a total utility of 0 in this simulation. Per definition in chapter \ref{ch:leverageCycle} each trade results in a utility-increase. If the agents start with the basic endowment of 1 unit of cash and 1 unit of assets as given in chapter \ref{ch:leverageCycle} then they will start already with a utility $\geq 0$. Thus it can never reach 0.

\medskip

\begin{proof}
\begin{align*}
	utility_{seller} = u_{asset} + u_{bond} + u_{cash}		\tag*{seller holds only assets, wants to sell them to buyer}
	\\ = u_{asset} + u_{cash}
	\\ = limit_{asset} \; holdings_{asset} + 
			(sell_{asset} - buy_{asset}) \; price_{asset} + \\
			(give_{bond} - take_{bond}) \; price_{bond} + \\
			holdings_{cash} 									\tag*{will sell only assets}
	\\ = limit_{asset} \; holdings_{asset} + \\
		(sell_{asset} \; price_{asset} + holdings_{cash}
\end{align*}
\end{proof}

\begin{proof}
\begin{align*}
	utility_{buyer} = u_{asset} + u_{bond} + u_{cash}		\tag*{buyer holds only cash, wants to buy assets from seller}
	\\ = u_{asset} + u_{cash}
	\\ = limit_{asset} \; holdings_{asset} + 
			(sell_{asset} - buy_{asset}) \; price_{asset} + \\
			(give_{bond} - take_{bond}) \; price_{bond} + \\
			holdings_{cash} 									\tag*{will buy only assets}
	\\ = limit_{asset} \; holdings_{asset} - buy_{asset} \; price_{asset} + holdings_{cash}
\end{align*}
\end{proof}
...
\begin{proof}
\begin{align*}
	utility_{before} = limit_{asset} \; holdings_{asset} + \\
					sell_{asset} \; price_{asset} + holdings_{cash} \tag*{no cash and nothing sold before} 
	\\ = limit_{asset} \; holdings_{asset}
\end{align*}
\end{proof}
...
\begin{proof}
\begin{align*}
	utility_{after} = limit_{asset} \; holdings_{asset} + \\
					sell_{asset} \; price_{asset} + holdings_{cash} \tag*{all assets sold and no cash before} 
...
\begin{proof}
\begin{align*}
	utility_{after} > utility_{before}
	sell_{asset} \; price_{asset} > limit_{asset} \; holdings_{asset}		\tag*{$| holdings_{asset} = sell_{asset}$} 
	sell_{asset} \; price_{asset} > limit_{asset} \; sell_{asset}
	\Rightarrow price_{asset} > limit_{asset}
\end{align*}
\end{proof}
...
\begin{align*}
	utility_{before} = limit_{asset} \; holdings_{asset} - \\
					buy_{asset} \; price_{asset} + holdings_{cash} \tag*{only cash and nothing bought before} 
	\\ = holdings_{cash}
\end{align*}
\end{proof}
...
\begin{align*}
	utility_{after} = limit_{asset} \; holdings_{asset} - \\
					buy_{asset} \; price_{asset} + holdings_{cash} \tag*{all assets bought against all cash} 
	\\ = limit_{asset} \; buy_{asset} - buy_{asset} \; price_{asset}
\end{align*}
\end{proof}
...
\begin{align*}
	utility_{after} > utility_{before}
	limit_{asset} \; buy_{asset} - buy_{asset} \; price_{asset} > holdings_{cash} \tag*{$|:buy_{asset} > 0$}
	limit_{asset} - price_{asset} > \frac{holdings_{cash}}{buy_{asset}}
	\Rightarrow limit_{asset} > price_{asset}
\end{align*}
\end{proof}
